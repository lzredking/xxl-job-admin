...
public class JobGroupController {
	...
	public ReturnT<String> save(XxlJobGroup xxlJobGroup){
		...
		for(XxlJobGroup group:groups){
			if(group.getAppName().equals(xxlJobGroup.getAppName())
					&& group.getAddressType()==xxlJobGroup.getAddressType()){
				isExist=true;
				groupId=group.getId();
			}
		}
		if(!isExist){
			...
			ReturnT<String> rt=new ReturnT<String>((ret>0)?ReturnT.SUCCESS_CODE:ReturnT.FAIL_CODE, xxlJobGroup.getId()+"");
			return rt;//(ret>0)?ReturnT.SUCCESS:ReturnT.FAIL;
		}else{
			ReturnT<String> rt=new ReturnT<String>(ReturnT.SUCCESS_CODE, groupId.toString());
			return rt;
		}
		//////////////////////////////	
	}
